...
def mutRandom(individual, manager, indpb1=0.15, indpb2=0.8):
    movien = len(manager.movies)
    halln = len(manager.halls)
    ...
    for k, hall in enumerate(individual):
        if random() < indpb1:
            for i in range(1, len(hall), 2):
                if random() < indpb2:
                    if random() < 0.7:
                        hall[i] = max(0, hall[i]-1) if hall[i] != 0 else hall[i]+1
                    else:
                        hall[i] = min(movien, hall[i]+1) if hall[i] != movien else hall[i]-1
        else:
